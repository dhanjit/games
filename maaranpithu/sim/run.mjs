 ...
 */
import { createWorld, step, T, PERSONAS } from '../rules.js';
...
let looseShare = 0, airgaps = 0, holdings = 0, throwsPerRound = 0, stalls = 0, untargeted = 0;
...
for (let r = 0; r < ROUNDS; r++) {
  ...
  while (!w.over && w.t < MAX_T) {
    for (const e of step(w, DT)) {
      if (e.type === 'throw') { pending = e.targetDist === null ? null : bucketOf(e.targetDist); if (e.targetDist === null) untargeted++; }
      else if (pending !== null && (e.type === 'hit' || e.type === 'caught' || e.type === 'airgap' || e.type === 'drop')) {
        outcomes[pending][e.type === 'drop' ? 'miss' : e.type]++; pending = null;
      }
    }
  }
  if (!w.over) stalls++;
